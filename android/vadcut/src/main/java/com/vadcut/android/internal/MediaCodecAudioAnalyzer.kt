...
internal class MediaCodecAudioAnalyzer(private val context: Context) {
    ...
    ): AnalysisResult = decode(inputUri, config, onProgress)

    /** Decodes the stream to obtain an exact duration without loading or running an activity detector. */
    suspend fun analyzeDuration(
        inputUri: Uri,
        onProgress: suspend (processedUs: Long, totalUs: Long) -> Unit,
    ): AnalysisResult = decode(inputUri, null, onProgress)
    ...
    private suspend fun decode(
        ...
        val detector = config?.let { currentConfig ->
            when (currentConfig.mode) {
                TrimMode.SPEECH -> SileroVadEngine(context, currentConfig.verifyModelIntegrity)
                TrimMode.NON_SILENCE -> EnergyActivityDetector(currentConfig.energyThresholdDb)
            }
        }
        ...
        try {
            ...
            val frames = if (config != null && detector != null) {
                VadFrameCollector(detector, ActivitySegmentPlanner(config))
            } else {
                null
            }
            ...
            while (!outputEnded) {
                ...
                when (val outputIndex = decoder.dequeueOutputBuffer(bufferInfo, CODEC_TIMEOUT_US)) {
                    MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> {
                        ...
                        if (decodedSourceFrames > 0L && newRate != outputSampleRate) {
                            throw TrimException(
                                TrimErrorCode.UNSUPPORTED_AUDIO_FORMAT,
                    ...
                    MediaCodec.INFO_TRY_AGAIN_LATER -> Unit
                        ...
                        if (bufferInfo.size > 0) {
                            ...
                            if (frames == null) {
                                decodedSourceFrames += pcmFrameCount(
                                ...
                                val outputBuffer = decoder.getOutputBuffer(outputIndex)
                                    ?: throw TrimException(
                                        TrimErrorCode.ANALYSIS_FAILED,
                                        "Decoder returned a null output buffer",
                                    )
            ...
            if (decodedSourceFrames == 0L || outputSampleRate <= 0) {
            ...
            val durationUs = maxOf(decodedDurationUs, frames?.outputDurationUs ?: 0L).coerceAtLeast(1L)
            AnalysisResult(
                durationUs = durationUs,
                keptRanges = frames?.finish(durationUs).orEmpty(),
    ...
    private fun consumePcm(
        ...
        val frameCount = pcmFrameCount(buffer.remaining(), channelCount, encoding)
        repeat(frameCount) {
            var mono = 0f
            repeat(channelCount) {
                mono += when (encoding) {
    ...
    private fun pcmFrameCount(byteCount: Int, channelCount: Int, encoding: Int): Int {
